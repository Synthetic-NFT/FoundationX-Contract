...
describe("#MockNFT", function () {
  ...
  it("Batch token URI", async function () {
    ...
    expect(await NFT.remainingTokenURI(BigNumber.from(0))).to.be.eql([
      [BigNumber.from(1), BigNumber.from(4), BigNumber.from(2)],
      ["1.png", "4.png", "2.png"],
    ]);
    expect(await NFT.remainingTokenURI(BigNumber.from(1))).to.be.eql([
    ...
    expect(await NFT.ownerOf(BigNumber.from(5))).to.be.equal(minter.address);
    ...
    expect(new Set(remainingIds)).to.be.eql(
      new Set([BigNumber.from(4), BigNumber.from(2), BigNumber.from(6)])
    );
    expect(new Set(remainingURIs)).to.be.eql(
      new Set(["4.png", "2.png", "6.png"])
    );
